...
function storeTestQueue() {
  if (!_storeTestQueue) {
    _storeTestQueue = new Queue(STORE_TEST_QUEUE, {
      ...
      defaultJobOptions: {
        // One retry on transient failures (Bright Data hiccup, CF flake,
        // Cloudflare challenge we couldn't solve on first pass). A second
        // attempt typically succeeds because the store itself works fine
        // — we just got an unlucky network moment. Two is the sweet spot:
        // more than that and a genuinely broken store eats 10+ minutes of
        // worker time retrying. Exponential backoff gives the upstream
        // service a second to recover before we hammer it again.
        attempts: 2,
...
function seqKeyForRun(runId) {
  return `run:${runId}:seq`;
}

// Each event gets a monotonic per-run sequence number (`_seq`) so SSE
// subscribers can resume cleanly after a reconnect: the client tracks
// the highest seq it has rendered (browsers send it back via
// `Last-Event-ID`), and the server's events endpoint filters out anything
// at-or-below that on replay.
async function publishEvent(runId, event) {
  const client = sharedRedis();
  const seq = await client.incr(seqKeyForRun(runId));
  const eventWithSeq = { ...event, _seq: seq };
  const payload = JSON.stringify(eventWithSeq);
  ...
  pipeline.expire(seqKeyForRun(runId), RUN_TTL_SECONDS);
...
//
// Ordering: subscribe BEFORE replay so events published mid-handshake
// land in `pending` rather than being dropped. After replay finishes
// we drain `pending` and switch to direct delivery. The caller is
// expected to deduplicate by `_seq` (publishEvent attaches a monotonic
// sequence number) since a fast publisher could land an event in both
// the list and the channel during the handshake window.
async function subscribeToRun(runId, onEvent) {
  ...
  // Now replay historical events from the list. The caller's
  // `_seq`-based dedup will skip anything already delivered.
  const historical = await shared.lrange(listForRun(runId), 0, -1);
  for (const raw of historical) {
    try { onEvent(JSON.parse(raw)); } catch (_) {}
  }

  // Drain any events that arrived during the handshake window.
  replayDone = true;
