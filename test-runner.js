  ...
  let loginDone = false;
    ...
    for (const testId of testIds) {
      ...
      try {
        ...
        // If the failure was a dead session, reconnect before the retry
        // so we don't just re-throw instantly on the same dead page.
        // Covers Playwright's "Target closed" family plus Bright Data's
        // WebSocket / network-layer disconnect messages.
        if (/Target (?:page|closed)|Browser has been closed|Execution context was destroyed|WebSocket|net::ERR|ECONNRESET|ECONNREFUSED|socket hang up|CDP session/i.test(err.message || '')) {
    ...
    // ── Release memory between tests (local mode only) ──
    // Heavy Shopify PDPs accumulate JS heap, image cache, and DOM state
    // in the same page across tests. After 4–5 tests this can balloon
    // past Railway's container memory limit, so navigating to
    // about:blank forces the renderer to drop the previous page.
    //
    // In remote mode we skip this: Bright Data manages memory server-
    // side, and the hardcoded 5s about:blank timeout races against
    // their 120s session lifecycle and causes false "session dead"
    // detections before `ensurePageAlive` can correctly classify the
